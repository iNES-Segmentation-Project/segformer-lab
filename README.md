...
>
> 본 프로젝트는 SegFormer-B0에서  
> decoder 구조(FPN)의 실제 기여를  
> controlled experiment와 decoder isolation 기반으로 분석한 연구 프로젝트입니다.
...
```mermaid
flowchart LR
    A["Research Hypothesis<br/>Decoder fusion effect<br/>under lightweight encoder"]
    B["Stage 1<br/>Controlled Experiment<br/>E0–E4"]
    C["Initial Evidence<br/>FPN +0.0147"]
    D["Stage 2<br/>Combined Experiment<br/>E5"]
    E["Pipeline Gain<br/>+0.1890"]
    F["Interpretation Limit<br/>Attribution issue"]
    G["Stage 2+<br/>Decoder Isolation"]
    H["Final Insight<br/>FPN retained<br/>+0.0176"]
...
G3는 Kvasir-SEG 기반 아래의 medical segmentation project에서 확인하실 수 있습니다.
...
> E1(FPN)에서 weak class 중심의 일관된 improvement 관찰  
> loss-based gain은 class-wise pattern이 상대적으로 불안정

<details>
<summary>Stage 1 Detailed Analysis</summary>

- decoder 구조 변경 효과 분석
- loss function generalization 비교
- small/thin object improvement 패턴 분석
- validation/test discrepancy 확인
...
- CamVid 외 domain 일반화는 별도 repo에서 검증 필요
