...
### 왜 decoder effect를 분석하려 했는가?

본 프로젝트에서는 단순히 성능 향상을 확인하는 것이 아닌,

- 왜 성능이 좋아졌는가
- decoder 구조 자체의 효과가 존재하는가
- validation improvement가 실제 test generalization으로 이어지는가

를 분석하는 방향에 집중했습니다.

특히 lightweight encoder에서는

> decoder 단계의 multi-scale feature fusion이  
> segmentation quality와 semantic propagation에 더 큰 영향을 줄 수 있다는 가설을 세웠습니다.
...
# 5. Stage 1 — Controlled Experiment
...
> lightweight encoder에서는  
> decoder 단계의 multi-scale feature fusion 영향이 더 커질 수 있다.

Stage 1 결과에서

- FPN decoder(E1)는 weak small/thin class 중심의 improvement를 보였습니다.
- loss 변경 기반 실험(E2–E4)은 class-wise gain이 불안정하게 나타났습니다.
...
같은 small/thin object에서 비교적 일관된 improvement를 보였습니다.

이는 FPN의 top-down pathway가  
deep semantic feature를 shallow high-resolution feature로 전달하면서

- semantic continuity 유지
- small object feature propagation
- boundary-aware interaction

에 도움을 주었을 가능성과 연결됩니다.

즉,

> decoder 구조 변경이 loss 변경보다 더 안정적인 **improvement signal**을 보였다는 점이  
> 이후 E5 combined experiment와 Stage 2+ decoder isolation 설계의 핵심 근거가 되었습니다.

---

# 6. Stage 2 — Combined Experiment (E5)
...
> "성능은 좋아졌지만, 왜 좋아졌는지는 아직 분리되지 않았다"
...
- FPN effect 때문인가?
- pretrained effect 때문인가?
- augmentation effect 때문인가?

이 문제를 해결하기 위해 Stage 2+ decoder isolation experiment를 추가 설계했습니다.

---

# 7. Stage 2+ — Decoder Isolation

## Why Stage 2+ Matters

본 프로젝트에서 중요한 점은

> 단순히 성능 향상을 확인한 것이 아니라,  
> combined experiment(E5)의 attribution limitation을 인지하고  
> decoder isolation experiment를 추가 설계했다는 점입니다.
...
같은 small/thin object에서 improvement가 집중적으로 나타났습니다.

이는

> FPN decoder의 multi-scale feature fusion과 top-down semantic propagation이  
...
> E5 improvement가 단순 pretrained 효과만으로 설명되기 어렵고,  
...
- loss 변경보다 decoder 구조 변경이 더 안정적인 pattern을 보였습니다.
- validation improvement와 test generalization은 항상 일치하지 않았습니다.
- combined experiment(E5)만으로는 attribution이 어렵다는 점을 확인했습니다.
- decoder isolation(Stage 2+)을 통해 FPN effect를 추가 검증했습니다.
- failure case(Bicyclist)를 통해 FPN effect가 universal하지 않을 가능성도 확인했습니다.
