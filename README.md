...
<p align="center">
  ...
  <br/>
<sub>
  <b>MiT encoder scale에 따라 segmentation 성능 차이가 발생</b><br/>
...
E2(Focal Loss) 실험에서 **validation 성능과 test 성능 방향이 불일치하는 케이스**가 관찰되어,  
validation 지표만으로 효과를 판단하는 것의 한계를 확인했습니다.  
...
E5 combined experiment에서 **+0.1890 mIoU**가 관찰되었지만,  
복수의 변수가 동시에 변경된 실험이었기 때문에 **FPN 단독 기여로 해석할 수 없었습니다.**

이를 보완하기 위해 decoder만 변경하는 Stage 2+ isolation experiment를 추가 설계했고,  
scratch/pretrained 두 조건 모두에서 **FPN 향상이 재현(+0.0147 → +0.0176)** 되었습니다.

단, **Bicyclist class에서는 오히려 성능이 감소(-0.0203)** 하여,  
FPN 효과가 모든 class에 보편적으로 작용하지 않는다는 점도 함께 확인했습니다.
...
loss 변경에 비해 **보다 일관된 양의 변화 패턴을 보인 경향**이 있음을 시사합니다.
...
> E0(MLP baseline)와 E5(combined pipeline) 간 boundary activation 비교입니다.  
> FPN 단독 효과가 아니라 pipeline 전체 조건의 차이를 반영한 결과이며,  
> 정량 결과를 보조하는 정성적 참고 자료로 활용했습니다.
...
> 이는 다른 조건을 고정했을 때 decoder 구조 자체의 기여 가능성이 존재한다는 근거로 해석할 수 있습니다.
...
Pole · SignSymbol · Pedestrian에서 **상대적으로 더 큰 향상이 관찰된 경향**은,  
...
FPN의 top-down pathway가 deep feature의 맥락 정보를 강하게 전파하는 과정에서,  
**Pedestrian · Pole 등 인접 class와의 경계가 혼동**되었을 가능성이 있습니다.

또한 CamVid dataset에서 Bicyclist는 **상대적으로 빈도가 낮은 class**이기 때문에,  
feature fusion의 이점이 충분히 작동하지 않았을 수 있습니다.

> 다만 위 두 가지 해석은 실험 내에서 직접 검증된 것이 아니라 가설 수준입니다.  
...
- **FPN decoder는 경량 encoder 환경에서 일관된 성능 향상 신호**를 보였습니다.  
  (scratch +0.0147 / pretrained +0.0176)

- **E5 combined experiment의 +0.1890은 특정 요소 단독 효과로 단정할 수 없었으며**,  
  Stage 2+ isolation을 통해 decoder 기여 가능성을 추가 검증했습니다.

- **Bicyclist failure case**를 통해,  
  FPN 효과가 모든 class에 동일하게 적용되지 않는다는 점이 관찰되었습니다.

- **E2 실험에서의 validation/test 괴리** 사례를 통해,  
  metric 수치만으로 실험 결과를 판단하는 데 한계가 있음을 확인했습니다.
