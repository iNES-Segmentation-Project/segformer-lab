...
E5 combined experiment에서 +0.1890 mIoU가 관찰되었지만, 복수의 변수가 동시에 변경된 실험이었기 때문에 **FPN 단독 기여로 해석할 수 없었습니다.**  
...
validation metric만으로는 실제 generalization을 충분히 설명하기 어렵다는 점도 실험 과정에서 확인되었습니다.  
E2(Focal Loss) 실험에서 validation 성능과 test 성능 간 discrepancy가 관찰되었고,  
E5에서는 수치상의 큰 향상에도 불구하고 attribution을 분리하지 않으면 결론이 달라질 수 있었습니다.  
...
> E1(FPN)은 Pole, SignSymbol, Pedestrian 등 작고 얇은 객체 class에서 비교적 일관된 성능 향상을 보였습니다.  
...
따라서 **+0.1890의 성능 향상을 특정 요소 하나의 효과로 해석할 수 없습니다.**

이 attribution limitation을 보완하기 위해 **Stage 2+ decoder isolation experiment**를 추가 설계했습니다.
...
E5의 attribution issue를 보완하기 위해 Stage 2+에서는 **decoder만 변경**했습니다.  
다른 모든 조건(pretrained encoder / CE loss / scheduler / augmentation / train-val-test split)은 고정했습니다.

> **"성능이 좋아졌는가?"가 아니라,  
> "무엇이 실제 성능 향상에 기여했는가?"를 검증하는 것**이 핵심 목적이었습니다.
...
FPN decoder의 **top-down semantic propagation**이 shallow feature의 부족한 semantic context를 deep feature로 보완하면서  
...
**Bicyclist class에서는 오히려 성능이 감소(-0.0203)** 했습니다.
...
FPN의 top-down pathway가 deep feature의 semantic context를 강하게 전파할 때,  
**Pedestrian · Pole 등 인접 class와의 경계가 혼동**되었을 가능성이 있습니다.  
또한 CamVid dataset에서 Bicyclist는 상대적으로 빈도가 낮은 class이기 때문에,  
multi-scale feature fusion의 이점이 충분히 작동하지 않았을 수 있습니다.
...
> E2(Focal Loss)에서 validation 성능과 test 성능 간 discrepancy가 관찰되었습니다.  
...
- **validation/test discrepancy** 분석을 통해, metric 수치만으로 실험 결과를 판단하는 데 한계가 있음을 확인했습니다.
